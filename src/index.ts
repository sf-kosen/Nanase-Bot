import { Client, GatewayIntentBits, ModalSubmitInteraction, ButtonInteraction, Interaction, CacheType } from "discord.js";
...
client.on("interactionCreate", async (interaction: Interaction<CacheType>) => { // ボタンの実行
    ...
    try {
        ...
        console.error(error);
        if (interaction.replied || interaction.deferred) {
            await interaction.followUp({ content: "There was an error while executing this action!", ephemeral: true });
        } else {
